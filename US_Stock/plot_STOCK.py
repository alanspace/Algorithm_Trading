...
# --- GPU/MPS Device Setup for TensorFlow ---
try:
    mps_devices = tf.config.list_physical_devices('MPS')
    if mps_devices:
        print("Apple Silicon MPS device found. Using MPS for TensorFlow.")
        # No need to set device manually; TensorFlow will use MPS automatically if available
        device_type = 'MPS'
    else:
        print("No MPS device found. Using CPU for TensorFlow.")
        device_type = 'CPU'
except Exception as e:
    print(f"Error checking for MPS device: {e}\nDefaulting to CPU.")
    device_type = 'CPU'
...
print("Num MPS Devices Available:", len(tf.config.list_physical_devices('MPS')))
print("MPS devices:", tf.config.list_physical_devices('MPS'))
print("TensorFlow device type in use:", device_type)
end_time = time.time()
